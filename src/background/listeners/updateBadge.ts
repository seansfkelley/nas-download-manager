...
import { TaskState, Settings } from "../../common/state";

export function updateBadge(
  settings: Settings,
  taskState: Pick<TaskState, "tasks" | "taskFetchFailureReason">,
): void {
  if (taskState.taskFetchFailureReason != null) {
    ...
    if (taskState.tasks == null) {
      ...
      taskCount = taskState.tasks.filter(
    ...
    browser.browserAction.setBadgeText({ text: taskCount === 0 ? "" : taskCount.toString() });
