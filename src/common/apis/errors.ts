...
const DOWNLOADSTATION_ERRORS: Record<string, string> = {
...
const FILESTATION_ERRORS: Record<string, string> = {
...
const ERROR_CODES = {
  ...
  [DownloadStation.Task.API_NAME]: DOWNLOADSTATION_ERRORS,
  [DownloadStation2.Task.API_NAME]: DOWNLOADSTATION_ERRORS,
  [FileStation.Info.API_NAME]: FILESTATION_ERRORS,
  [FileStation.List.API_NAME]: FILESTATION_ERRORS,
};

export function getErrorForFailedResponse(
  response: ClientFailureResponse,
  defaultMessage: string | null = "Unknown error.",
) {
  return (
    ERROR_CODES[response.apiGroup]?.[response.error.code] ||
