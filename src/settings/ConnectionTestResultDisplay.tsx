...
export class ConnectionTestResultDisplay extends React.PureComponent<Props, {}> {
  render() {
    ...
    if (testResult === 'none') {
      ...
      return this.renderResult(errorMessageFromCode(testResult.code, 'Auth'), 'fa-times', 'intent-error');
    } else {
      return this.renderResult(errorMessageFromConnectionFailure(testResult), 'fa-times', 'intent-error');
    }
  }
