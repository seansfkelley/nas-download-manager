...
Yes. The HTTP option was removed in version 0.12.0 to comply with a Firefox add-on policy requiring encrypted transport, and was restored once that policy was relaxed. The protocol dropdown next to the hostname in the settings page selects between the two.

HTTPS remains the default and the recommended choice, since it's the only one of the two that encrypts your credentials in transit. If you use it, note that you may need to [configure your certificates](#what-is-an-invalid-certificate) to allow NAS Download Manager to log in properly.
